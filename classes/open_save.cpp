...
void CreateInfoHeader(unsigned char *info_header, const int file_size, int width, int height, int x_pixels,
    ...
    info_header[1] = 0;  // NOLINT
    info_header[2] = 0;  // NOLINT
...
void SaveFile(const char *path, Image &image) {
    ...
    const int file_size = FILE_HEADER_SIZE + INFO_HEADER_SIZE + image.Width() * image.Height() * 3 +
                          count_paddings * image.Height();  // NOLINT
...
void Image::Read(const char *path) {
    ...
    for (int y = 0; y < height_; ++y) {
        for (int x = 0; x < width_; ++x) {
            ...
            pixels_[y][x].r = static_cast<double>(pixel_color[2]) / 255.0f;  // NOLINT
            pixels_[y][x].g = static_cast<double>(pixel_color[1]) / 255.0f;  // NOLINT
            pixels_[y][x].b = static_cast<double>(pixel_color[0]) / 255.0f;  // NOLINT
        }
